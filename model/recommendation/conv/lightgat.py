...
class LightGAT(torch.nn.Module):
    def __init__(self, config):
        ...
        self.train_edge_index = torch.cat([self.user_spot, self.spot_user], dim=1)
        self.all_embedding = torch.nn.Embedding(num_embeddings=self.m_items+self.n_users, embedding_dim=self.hidden_channels) #[spot, user]
...
if __name__=='__main__':
    ...
    model = LightGAT(config).to(device)
