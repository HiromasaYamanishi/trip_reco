...
class HeteroSAGELightConv(torch.nn.Module):
    def __init__(self, in_channels_dict, edge_index_dict, out_channels, ReLU):
        super().__init__()
...
class HeteroSAGE(torch.nn.Module):
    def __init__(self, data, config, out_channels=1,multi=True):
        ...
        self.layers.append(HeteroSAGEConv(self.first_in_channels_dict, edge_index_dict, self.hidden_channels, self.ReLU))

        for i in range(self.num_layers-1):
            self.layers.append(HeteroSAGEConv(self.mid_in_channels_dict, edge_index_dict, self.hidden_channels, self.ReLU))
...
class HeteroSAGEEmb(torch.nn.Module):
