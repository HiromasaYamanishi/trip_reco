...
class MultiDenoiseGCN(torch.nn.Module):
    def __init__(self, config):
        ...
        self.dist_conv = GCNLightConv(self.hidden_channels, self.hidden_channels, add_self_loops=False, bias=False)
        ...
        self.dist_edge_index = torch.load('/home/yamanishi/project/trip_recommend/model/recommendation/data/spot_dist_spot_edge_index.pt').to(self.device)
        self.dist_edge_weight = torch.load('/home/yamanishi/project/trip_recommend/model/recommendation/data/spot_dist_spot_edge_weight.pt').to(self.device)
        print('loaded dist A')
        ...
        self.category_edge_index = torch.load('/home/yamanishi/project/trip_recommend/model/recommendation/data/spot_category_spot_edge_index.pt').to(self.device)
        self.category_edge_weight = torch.load('/home/yamanishi/project/trip_recommend/model/recommendation/data/spot_category_spot_edge_weight.pt').to(self.device)
    ...
    def forward(self):
        ...
        #spot_cat_x = self.category_conv(spot_x, self.category_edge_index, self.category_edge_weight)
        spot_dist_x = self.dist_conv(spot_x, self.dist_edge_index, self.dist_edge_weight)
        #spot_cit_x = self.city_conv(spot_x, self.city_edge_index, self.city_edge_weight)
        spot_x = spot_x + spot_dist_x

        user_x_user = self.user_conv(user_x, self.user_edge_index, self.user_edge_weight)
        user_x = user_x + user_x_user
        spot_x_out = spot_x
        user_x_out = user_x
        del spot_dist_x, user_x_user
        for i in range(self.num_layers):
