...
class HeteroGGNNConvV2(torch.nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        for k,v in edge_index_dict.items():
            ...
            target_x =  self.gru['__'.join(k)](aggregated, target_x)
...
class HeteroGGNNV2(torch.nn.Module):
    def __init__(self, data, config, out_channels=1,multi=True):
        ...
        self.layers.append(HeteroGGNNConvV2(self.first_in_channels_dict, edge_index_dict, self.hidden_channels, self.ReLU))

        for i in range(self.num_layers-1):
            self.layers.append(HeteroGGNNConvV2(self.mid_in_channels_dict, edge_index_dict, self.hidden_channels, self.ReLU))
...
if __name__=='__main__':
    ...
    model = HeteroGGNN(data, num_layers=5,hidden_channels=128, out_channels=1,multi=True)
    data.to(device)
    model.to(device)
    print(model)
