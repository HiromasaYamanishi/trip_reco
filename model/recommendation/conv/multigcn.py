...
class MultiGCN(torch.nn.Module):
    def __init__(self, config):
        ...
        self.user_edge_index = torch.load('/home/yamanishi/project/trip_recommend/model/recommendation/data/user_user_edge_index.pt')
        self.user_edge_weight = torch.load('/home/yamanishi/project/trip_recommend/model/recommendation/data/user_user_edge_weight.pt')
        print('loaded user A')
        ...
        self.city_edge_index = torch.load('/home/yamanishi/project/trip_recommend/model/recommendation/data/spot_city_spot_edge_index.pt')
        self.city_edge_weight = torch.load('/home/yamanishi/project/trip_recommend/model/recommendation/data/spot_city_spot_edge_weight.pt')
        print('loaded city A')
        ...
        self.station_edge_index = torch.load('/home/yamanishi/project/trip_recommend/model/recommendation/data/spot_station_spot_edge_index.pt')
        self.station_edge_weight = torch.load('/home/yamanishi/project/trip_recommend/model/recommendation/data/spot_station_spot_edge_weight.pt')
        ...
        self.category_edge_index = torch.load('/home/yamanishi/project/trip_recommend/model/recommendation/data/spot_category_spot_edge_index.pt')
        self.category_edge_weight = torch.load('/home/yamanishi/project/trip_recommend/model/recommendation/data/spot_category_spot_edge_weight.pt')
    ...
    def forward(self):
        ...
        for i in range(self.num_layers):
            ...
            if self.config['model']['mid']:
                ...
                spot_x_cit = self.city_conv(spot_x, self.city_edge_index, self.city_edge_weight)
                spot_x_sta = self.station_conv(spot_x, self.station_edge_index, self.station_edge_weight)
                cat_attention_score = self.biattn_mid(spot_x, spot_x_cat)
                cit_attention_score = self.biattn_mid(spot_x, spot_x_cit)
                sta_attention_score = self.biattn_mid(spot_x, spot_x_sta)
                cat_attention_score, cit_attention_score, sta_attention_score = self.biattn_mid.calc_attention(cat_attention_score, cit_attention_score, sta_attention_score)
                spot_x_cat = spot_x_cat * cat_attention_score
                spot_x_cit = spot_x_cit * cit_attention_score
                spot_x_sta = spot_x_sta * sta_attention_score
                #spot_x_cat = self.category_conv(spot_x, self.spot_category_spot_sparse)
                #spot_x_cit = self.city_conv(spot_x, self.spot_city_spot_sparse)
                #spot_x_sta = self.station_conv(spot_x, self.spot_station_spot_sparse)
        ...
        if self.config['model']['post']:
            ...
            cat_attention_score = self.biattn_post(spot_x, spot_x_cat)
            cit_attention_score = self.biattn_post(spot_x, spot_x_cit)
            sta_attention_score = self.biattn_post(spot_x, spot_x_sta)
            ...
            #spot_x_cat = spot_x_cit
            if self.config['model']['cat']:
                spot_x_out = torch.cat([spot_x, (spot_x_cat + spot_x_cit + spot_x_sta)/3], dim=1)
                user_x_out = torch.cat([user_x_out, user_x_out], dim=1)
            else:
                #spot_x_out = spot_x_out +  (spot_x_cat+spot_x_cit+spot_x_sta)/3
                spot_x = spot_x_out + (spot_x_cat+spot_x_cit+spot_x_sta)
            del spot_x_cat, spot_x_cit, spot_x_sta
            del spot_x, user_x
    ...
    def bpr_loss(self, spot_x_out, user_x_out, users, pos, neg):
        ...
        pos_pop = 1+0.1*torch.log(torch.log(self.popularity.to(self.device)[pos]))
        neg_pop = 1+0.1*torch.log(torch.log(self.popularity.to(self.device)[neg]))
        pos_scores = torch.sum(torch.mul(users_emb, pos_emb), dim=1)
        neg_scores = torch.sum(torch.mul(users_emb, neg_emb), dim=1)

        loss = torch.mean(torch.nn.functional.softplus((neg_scores-pos_scores)*(1+neg_pop-pos_pop)))
...
if __name__=='__main__':
    ...
    model = MultiGCN(config).to(device)
