...
class Transformer(torch.nn.Module):
    def __init__(self, input_dim, num_heads, split,):
        ...
        self.ln1 = nn.LayerNorm(input_dim*self.split)
        self.do1 = nn.Dropout(0.2)
        self.ffn = FFN(input_dim * self.split, input_dim * self.split)
        self.ln2 = nn.LayerNorm(input_dim*self.split)
...
class MyHetero(torch.nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict):
        if self.multi == True:
            torch.mean(self.att(x_dict['spot'].reshape(-1,5,512)), dim=1)
        ...
        for k in x_dict.keys():
            x_dict[k] = self.linears[k](x_dict[k])
        
        return x_dict



def train(model, optimizer, data, alpha=1):
    model.train()
    optimizer.zero_grad()
    ...
    if isinstance(out, dict):
        ...
        loss_spot = F.mse_loss(out['spot'][spot_mask].flatten(), data['spot'].y[spot_mask])#/spot_mask.sum()
        city_mask = data['city'].train_mask
        loss_city = F.mse_loss(out['city'][city_mask].flatten(), data['city'].y[city_mask])#/city_mask.sum()
        pref_mask = data['pref'].train_mask
        loss_pref = F.mse_loss(out['pref'][pref_mask].flatten(), data['pref'].y[pref_mask])#/pref_mask.sum()
...
def test(model,data):
    ...
    for split in ['train_mask', 'valid_mask', 'test_mask']:
        ...
        if isinstance(pred, dict):
            ...
            loss_spot = F.mse_loss(pred['spot'][spot_mask].flatten(), data['spot'].y[spot_mask])#/mask.sum()
            city_mask = data['city'][split]
            loss_city = F.mse_loss(pred['city'][city_mask].flatten(), data['city'].y[city_mask])#/mask.sum()
            pref_mask = data['pref'][split]
            loss_pref = F.mse_loss(pred['pref'][pref_mask].flatten(), data['pref'].y[pref_mask])#/mask.sum()
            loss = alpha*loss_city+loss_spot
        else:
...
if __name__=='__main__':
    ...
    data = get_data(df, category=True, city=True, prefecture=True, multi=True)
    ...
    train_epoch(model, data, epoch_num=500)
    #torch.save(model.state_dict(), '../data/model/model_new.pth')
    spot_cor, city_cor = calc_cor_save(model, data)
