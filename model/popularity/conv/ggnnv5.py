...
class HeteroGGNNConv(torch.nn.Module):
    def __init__(self, in_channels_dict, edge_index_dict, out_channels, ReLU):
        ...
        self.gru1 = nn.ModuleDict({})
        for k in edge_index_dict.keys():
            self.gru1['__'.join(k)] = nn.GRUCell(out_channels, out_channels)
        self.gru2 = nn.ModuleDict({})
        for k in edge_index_dict.keys():
            self.gru2['__'.join(k)] = nn.GRUCell(out_channels, out_channels)
        self.ReLU = ReLU
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        for k,v in edge_index_dict.items():
            ...
            target_x =  self.gru1['__'.join(k)](target_x, aggregated)
            target_x = self.gru2['__'.join(k)](target_x, aggregated)
...
class HeteroGGNNV5(torch.nn.Module):
    def __init__(self, data, config, out_channels=1,multi=True):
        ...
        self.layers.append(HeteroGGNNConv(self.first_in_channels_dict, edge_index_dict, self.hidden_channels, self.ReLU))

        for i in range(self.num_layers-1):
            self.layers.append(HeteroGGNNConv(self.mid_in_channels_dict, edge_index_dict, self.hidden_channels, self.ReLU))
