...
class HeteroLSTMConv(torch.nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        for k,v in edge_index_dict.items():
            ...
            '''
            sorted_index, index = torch.sort(target_index)
            source_x = source_x[index]
            batch_size = 1
            aggregated_all = []
            sort_unique = torch.unique(target_index)
            for i in range(0, len(sort_unique),batch_size):
                unique = sort_unique[i:min(i+batch_size, len(sort_unique))]
                sorted_index_tmp = sorted_index[torch.isin(sorted_index, unique)]
                source_x_tmp = source_x[torch.isin(sorted_index, unique)]
                print(len(source_x_tmp))
                aggregated = aggr(source_x_tmp, sorted_index_tmp)
                aggregated_all.append(aggregated)
                del aggregated, unique, source_x_tmp, sorted_index_tmp
            aggregated_all = torch.cat(aggregated_all, dim=0)
