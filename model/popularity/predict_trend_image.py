...
class PlaceDataset(Dataset):
    ...
    def __init__(self, transform, mode) -> None:
        super().__init__()
        self.path = Path()
        self.df = pd.read_csv(self.path.df_experience_path)
        self.mode = mode
        if mode=='train':
            ...
            self.df = self.df[self.df['valid']==1].reset_index(drop=True)
        self.spots = self.df['観光地名'].values
        #self.page_view = self.df['page_view'].values/self.df['page_view'].max()
        self.page_view = np.log10(self.df['jalan_review_count']+1)
        self.transform = transform

    def __getitem__(self, index):
        spot_name = self.spots[index]
        img_file_path = os.path.join(self.path.jalan_image_dir , spot_name + '.jpg')
