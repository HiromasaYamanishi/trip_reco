...
from dataloader_food import get_content_graph, get_citystation_graph, get_geo_graph, get_train_graph
...
class DeepTour(torch.nn.Module):
    def __init__(self, config):
        ...
        self.content_graph = get_content_graph(config).to(self.device)
        print(self.content_graph)
        #self.content_gnn = DeepTourModel(self.content_graph, config).to(self.device)
        self.content_gnn = HeteroGGNN(self.content_graph, config, multi=True).to(self.device)
