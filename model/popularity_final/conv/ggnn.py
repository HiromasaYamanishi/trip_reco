...
class HeteroGGNN(torch.nn.Module):
    def __init__(self, data, config, out_channels=1,multi=True):
        ...
        if multi==True:
            self.att = AttentionModule(input_dim=512, split=config['model']['split'])
            self.first_in_channels_dict['spot'] = 512
       
        self.layers.append(HeteroGGNNConv(self.first_in_channels_dict, edge_index_dict, config))

        for i in range(self.num_layers-1):
            self.layers.append(HeteroGGNNConv(self.mid_in_channels_dict, edge_index_dict, config))
