...
class HeteroGGNNConvV4(torch.nn.Module):
    def __init__(self, in_channels_dict, edge_index_dict, out_channels, ReLU):
        ...
        for k in edge_index_dict.keys():
            self.gru_meta['__'.join(k)] = nn.GRUCell(out_channels, out_channels)
...
class HeteroGGNNV4(torch.nn.Module):
    def __init__(self, data, config, out_channels=1,multi=True):
        ...
        self.layers.append(HeteroGGNNConvV4(self.first_in_channels_dict, edge_index_dict, self.hidden_channels, self.ReLU))

        for i in range(self.num_layers-1):
            self.layers.append(HeteroGGNNConvV4(self.mid_in_channels_dict, edge_index_dict, self.hidden_channels, self.ReLU))
...
if __name__=='__main__':
    ...
    model = HeteroGGNNV4(data, config)
    data.to(device)
    model.to(device)
    print(model)
