...
class HeteroGGNNConvV3(torch.nn.Module):
    def __init__(self, in_channels_dict, edge_index_dict, out_channels, ReLU):
        ...
        for node_type in in_channels_dict.keys():
            self.gru_meta[node_type] = nn.GRUCell(out_channels, out_channels)
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        for node_type in x_dict.keys():
            ...
            for edge_type in edge_type_list:
                if edge_type[-1]!=node_type:continue
                ...
                if aggregate_meta==None:
                    ...
                    aggregate_meta+=aggregated
                target_x =  self.gru['__'.join(edge_type)](target_x, aggregated)
            if x_dict_out.get(target)!=None:
                x_dict_out[target] += target_x
            else:
                x_dict_out[target] = target_x
            aggregate_meta=aggregate_meta/self.div[node_type]
            aggregated_dict[node_type]=aggregate_meta
        x_dict_out = {k: v/self.div[k] for k,v in x_dict_out.items()}
        #x_dict_out = {k: self.gru_meta[k](aggregated_dict[k], v) for k,v in x_dict_out.items()}
        #aggregate_meta=aggregate_meta/self.div[node_type]
        #x_dict_out[node_type] = self.gru_meta[node_type](aggregate_meta, x_dict_out[node_type])
...
class HeteroGGNNV3(torch.nn.Module):
    def __init__(self, data, config, out_channels=1,multi=True):
        ...
        self.layers.append(HeteroGGNNConvV3(self.first_in_channels_dict, edge_index_dict, self.hidden_channels, self.ReLU))

        for i in range(self.num_layers-1):
            self.layers.append(HeteroGGNNConvV3(self.mid_in_channels_dict, edge_index_dict, self.hidden_channels, self.ReLU))
...
if __name__=='__main__':
    ...
    model = HeteroGGNNV3(data, config)
    data.to(device)
    model.to(device)
    print(model)
