...
class HGANConv(torch.nn.Module):
    def __init__(self, in_channels_dict, edge_index_dict, config):
        ...
        self.query = torch.nn.Parameter(torch.rand(self.out_channels*2))
        torch.nn.init.normal_(self.query, std=0.1)
        self.LeakyReLU = torch.nn.LeakyReLU(0.2)
        self.group_dict = torch.nn.ParameterDict()
        for k in self.div.keys():
            self.group_dict[k] = torch.nn.Parameter(torch.rand(self.out_channels))
            torch.nn.init.normal_(self.group_dict[k], std=0.1)
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        for k,v in edge_index_dict.items():
            ...
            att_value = self.LeakyReLU(torch.matmul(x, self.query))
            att_value = softmax(att_value, target_index).unsqueeze(1)
        ...
        att = {k: torch.sum(v*self.group_dict[k], dim=-1) for k,v in x_dict_out.items()} #[node_size, relation_num]
