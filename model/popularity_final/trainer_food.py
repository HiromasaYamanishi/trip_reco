...
import csv
import wandb
from model_food import DeepTour
import yaml
...
class Trainer:
    def __init__(self, config,):
        ...
        self.df = pd.read_csv('/home/yamanishi/project/trip_recommend/data/jalan/food/food_info_all_10000.csv')
        self.spot_names = self.df['food_name'].values
        self.max_cor = 0
    ...
    def test(self, model):
        ...
        y = data['spot'].y.to(self.device)
        for split in ['train_mask', 'valid_mask', 'test_mask']:
            #if split!='test_mask':
            #    mask = torch.cat([data['spot'][split] , torch.tensor([True]*10+[False]*50)])
            #else:
            #    mask = torch.cat([data['spot'][split] , torch.tensor([False]*10+[True]*50)])
        ...
        for epoch in range(epoch_num):
            ...
            if epoch%1==0:
                ...
                if cor>self.max_cor:
                    ...
                    #print('saved emb')
                    #model.eval()
                    torch.save(model.state_dict(), '/home/yamanishi/project/trip_recommend/model/popularity_final/data/deepfood.pth')
                    #print('saved model')
    ...
    def save_result(self, model):
        ...
        spot_names = self.df['food_name'].values[data['spot'].test_mask.cpu().numpy()]
        ...
        plt.savefig('/home/yamanishi/project/trip_recommend/model/popularity_final/data/scatter_food.png')
    ...
    def calc_cor(self, model):
        ...
        for split in ['test_mask']:
            ...
            if isinstance(out, dict):
                ...
                pred = out[mask].reshape(-1).cpu().numpy()
            cor = np.corrcoef(gt, pred)[0][1]
            #last_cor = np.corrcoef(gt[-50:], pred[-50:])[0][1]
            #print(gt[-60:])
            #print(pred[-60:])
            last_cor=0
        return cor, last_cor
...
if __name__=='__main__':
    ...
    config['model']['hidden_channels'] = 512
    config['model']['split']=3
    config['trainer']['lr'] = 5e-4
    config['data']['spot'] = False
    print(config)
    gnn = DeepTour(config)
    trainer = Trainer(config)
    trainer.train_epoch(gnn, epoch_num=300)
    #for i in range(3):
