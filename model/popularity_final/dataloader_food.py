...
class Path:
    def __init__(self):
        ...
        self.data_graph_dir = '/home/yamanishi/project/trip_recommend/data/jalan/graph/food'
...
def get_train_graph(config, model_name='ResNet', multi=True):
    df = pd.read_csv('/home/yamanishi/project/trip_recommend/data/jalan/food/food_info_all_10000.csv')
    df['y'] = np.log10(df['review_count'])
    mask = np.load('/home/yamanishi/project/trip_recommend/data/jalan/graph/food/valid_idx.npy')
    print('mask is', mask) 
...
def get_content_graph(config, model_name='ResNet', multi=True):
    ...
    jalan_graph_dir = '/home/yamanishi/project/trip_recommend/data/jalan/graph/food'
    data = HeteroData()
    ...
    if city:
        data['city'].x = torch.from_numpy(np.load(os.path.join(jalan_graph_dir, 'city_attr.npy'))).float()#torch.rand(city_size, 5)
