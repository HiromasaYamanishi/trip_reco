...
class QNet(torch.nn.Module):
    def __init__(self, data, hidden_dim=128):
        ...
        self.l1 = nn.Linear(self.emb_dim, self.hidden_dim)
...
class POIEnv:
    def __init__(self, data, device):
        ...
        self.word_emb = self.data['word'].x
        ...
        self.words = self.data.edge_index_dict['spot','relate','word'][1][self.data.edge_index_dict['spot','relate','word'][0]==node_idx]
        self.spot_word_emb = self.word_emb[self.words]
        self.spot_word_emb_mean = torch.mean(self.spot_word_emb, axis=0)
        self.spot_img_emb = self.data['spot'].x[node_idx]
        self.state = torch.cat([self.spot_word_emb_mean, self.spot_img_emb])
        self.word_num = 15
    ...
    def step(self, action):
        ...
        self.spot_word_emb = torch.cat([self.spot_word_emb, self.word_emb[action].unsqueeze(0)], axis=0)
        self.spot_word_emb_mean = torch.mean(self.spot_word_emb, axis=0)
        next_state = torch.cat([self.spot_word_emb_mean, self.spot_img_emb])
...
if __name__=='__main__':
    device='cuda:5'
    wandb.init(project='reinforcement poi',name='train')
    simulation(device)
